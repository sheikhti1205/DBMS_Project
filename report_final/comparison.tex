...
public Group-08 snapshot contains 50 files in \texttt{Datasets}, 47 CSV
...
categories total 126 visible leaf files. The file-type check finds 42 XLSX
files, 14 native Google Sheets, 47 CSV files, 2 PNG files, 18 JPEG files and 3
PDF files.
...
stated source-corpus definition & 50 files in \texttt{Datasets}: 41 XLSX and 9
...
ER or model artifacts & Approved final ERD and implementation contract; no
repository-total artifact count is used & 2 ER Diagram PNG files and 18
individual-model JPEG files \\
Files reaching an implementation & 9 structured files produce 62 blocks loaded
into the final implementation & Not established from the supplied evidence \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Model evidence & Approved final ERD plus a machine-readable schema contract &
...
Group-07's source audit also reads 3,717,305 candidate measurement cells,
