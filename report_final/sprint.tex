...
The work is organized into three sprints that move from source discovery to a
verified SQLite release. Each report uses the same structure: overview,
...
Bangladesh organizations and recognized international data portals, then
...
\mSourceOrganisations{} organizations finally contribute the
\mSourceFiles{} structured loader files used by the implementation. The other
...
\begin{enumerate}[leftmargin=*]
  \item Search national and international organizations for relevant
...
Verification and Selection. Completion requires both an organized collection
...
File organization & Sheikh Tawsif Bin Aftab and Syed Mehraz Monwar organized
...
Review and verification & Syed Mehraz Monwar checked the organization and
...
\begin{description}[style=nextline]
  ...
  before selection.
  \item[Source recording.] The register preserves the organization, material
  ...
  \item[Collection and organization.] Downloaded files are grouped by source
  organization in the shared Drive. Original material is preserved while
...
organizations and portals. & Maintain one source register with organization,
...
Sprint 1 produces a source register and an organized collection containing
...
offers \mRowsOffered{} rows across \mSourceBlocks{} implemented source blocks;
...
Monwar extracted the approved blocks and organized their raw CSV outputs. \\
...
\begin{enumerate}[leftmargin=*]
  ...
  outputs.
  \item Validate required values, key uniqueness, constraints, foreign keys and
...
file but a release whose schema, data and operating procedures can be rebuilt
and checked from the delivered project.
Figure~\ref{fig:sprint3-goals} shows the full set of goals completed in this
release sprint.
...
implemented tables, relationships and query behavior. \\
...
\begin{description}[style=nextline]
  ...
  agreement, primary-key uniqueness, required values, constraint behavior,
  foreign-key consistency and logical equivalence after a clean rebuild.
  ...
  read-only access, safe rebuild behavior, backup and restore are exercised
...
CQ3 & Can every loaded water-quality row reach a river through its station? &
...
\begin{lstlisting}[language=SQL,caption={Water-quality records joined to their rivers},
...
\midrule
\inputrows{generated/competency_results.tex}
\bottomrule
\end{tabular}
\end{table}
