...
The final ERD, schema definition, normalized outputs and SQLite database are
checked against the same relation and key definitions. Automated tests rebuild
the database, run integrity and foreign-key checks, execute the competency
queries and compare backup-restored contents. The current database returns
\texttt{\mDbIntegrity} for \texttt{PRAGMA integrity\_check} and
\mDbFkViolations{} foreign-key violations.
...
The database retains one industry category, Agriculture Forestry and Fishing,
and only \mIndustryRows{} fiscal
...
Local file-based SQLite deployment &
