...
applies a stricter condition: for every non-trivial functional dependency
\(X \rightarrow Y\), the determinant \(X\) is a superkey
\cite{kent1983five}.
...
Check that every non-trivial determinant is a superkey};
...
The team uses this candidate groundwater structure as a working 3NF example
during schema refinement. The dataset is later excluded because the final ERD
contains no groundwater relation. The example therefore documents the
transitive-dependency check performed during schema refinement rather than a
relation loaded into the final database.
...
when, for every non-trivial functional dependency \(X \rightarrow Y\), \(X\) is a
superkey \cite{codd1972further}. A determinant is an attribute, or a group of
attributes, that determines another attribute. A candidate key is a minimal
superkey, so the candidate keys identified for the project relations provide
the superkeys checked at this stage.
...
\textbf{Meaning} \\
\midrule
Dinajpur & 2011 & 1 & 9.2  & Minimum \\
Dinajpur & 2011 & 1 & 20.9 & Maximum \\
\bottomrule
\end{tabular}
...
\textbf{Temperature} \\
\midrule
Dinajpur & 2011 & 1 & Minimum & 9.2 \\
Dinajpur & 2011 & 1 & Maximum & 20.9 \\
\bottomrule
\end{tabular}
...
for the same month. Temperature Type is therefore required. When several
sources provide competing values for the same station, year, month and type,
the documented source-precedence rule resolves the competing observations
during reconciliation. After this reconciliation,
\((Station\_Name, Year, Month, Type)\) uniquely identifies the retained
temperature record and is used as the final primary key. This corrects an
incomplete proposed key; it is not presented as a BCNF violation or
decomposition. The BCNF test is applied only after the candidate keys have been
established.
...
For the industry example, the retained source contains one category,
Agriculture Forestry and Fishing, with produced-wastewater quantity for three
fiscal periods; the stored reuse percentage is derived from the published
produced and reused volumes, so \texttt{Quantity} is wastewater volume and is
not a count of establishments or a directly published percentage. This
explains the functional dependency above without changing the final schema.
