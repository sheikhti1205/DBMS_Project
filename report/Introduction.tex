...
A person can interpret these layouts visually, but a query cannot. The
...
value, the same query can be evaluated against one consistent set of
identifiers and stored values.
...
\begin{description}[leftmargin=1.15cm,style=nextline]
  ...
  merge across cells and a row's identity can depend on a label above it. If
  loaded directly as a relation, this structure contains repeating groups and
  does not satisfy First Normal Form. It also does not provide a dependable
  candidate key.
...
\begin{description}[leftmargin=0.6cm,style=nextline]
  ...
  grain. Solar radiation retains the source observation-hour index in
  \texttt{Sample\_No} because several readings occur on the same calendar day;
  the source labels this field ``Date''.
  ...
  \item[Industry] Wastewater observations are retained by establishment-size
  or industry category and fiscal period. Establishment-size rows store
  wastewater volume and the published share; industry rows store
  produced-wastewater quantity and a derived reuse percentage. The retained
  industry source contains one category, Agriculture Forestry and Fishing.
...
Source discovery covers Bangladeshi and international organisations. The
processing pipeline uses nine selected structured files from Bangladesh Bureau
of Statistics (BBS), Bangladesh Meteorological Department (BMD), Bangladesh Rice
...
Table~\ref{tab:organisations} adapts the selected-source profile to the
verified Drive inventory. Collected files are source or data publications; the
processing-input count excludes support files and derived page images.
...
\caption{Selected source profile within the collected file inventory}
\label{tab:organisations}
\begin{tabular}{L{0.33\textwidth}r r L{0.36\textwidth}}
\toprule
Organisation & Collected & \shortstack{Processing\\ inputs} & Selected material \\
\midrule
Bangladesh Bureau of Statistics (BBS) & 2 & 1 & Climate, water quality,
forest and industrial statistics. \\
Bangladesh Meteorological Department (BMD) & 11 & 2 & Temperature data and a
sunshine publication; only the temperature records contribute loadable
observations. \\
...
normalization and modelling. Deployment, collaboration, findings, pedagogy and
the evidence-based resource comparison follow before the conclusion and
bibliography.
