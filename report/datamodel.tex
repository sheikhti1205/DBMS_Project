...
\includegraphics[width=\linewidth,height=0.90\textheight,keepaspectratio]
  {assets/Final_erd.png}
\caption{Approved final Group-07 environmental ERD in BCNF. Source: Group-07
final Excalidraw drawing.}
\label{fig:final-erd}
\end{figure}
\end{landscape}
\restoregeometry
...
\bottomrule
\endfoot
\inputrows{scripts/generated/relations.tex}
\end{longtable}
...
radiation also retains the source observation-hour index in
\texttt{Sample\_No} because several readings occur on the same calendar day;
the source labels this field ``Date''. Reference relations use descriptive
names as keys so observation tables can point to them without repeating the
same text.
...
establishment-size and industry-usage records. \\\bottomrule
\end{tabular}
\end{table}
...
review & All \mWaterQualityConnected{} water-quality rows connect to a retained
river or water-body reference. \\
...
rows. \\
Water-quality units fixed by parameter & Units differ by measured quantity
(e.g.\ pH, mg/L, parts per thousand or item counts) & Measurement units are
documented in the normalization evidence rather than repeated in every
\texttt{Water\_Quality} row. \\
