...
Reference registers are structured where needed, while narrative publications
remain supporting evidence for headings, units, methodology and source
interpretation. Candidate entities
identified from the reviewed source structures are compared and consolidated
into one integrated Entity--Relationship model. The retained material is then
reshaped, reconciled
...
foreign-key violations. The database therefore reduces manual name
reconciliation across several spreadsheets: the same query can be evaluated
against one consistent set of identifiers and stored values, which supports
comparison, teaching and further environmental research.
